...
def write_predictions(model,data,tag2idx,config):
    '''Run prediction for one data point'''
    predictions=[]
    true=[]
...
def predict(model, config, data,epoch, tag2idx):
    ...
    print('True',true)
    print('Pred',pred)
    pickle.dump(true, open(config.path_to_saved_model + "true_best", "wb"))
    pickle.dump(pred,open(config.path_to_saved_model + "pred_best", "wb"))
...
def predict_for_all_epochs(model, config, data, tag2idx):
    ...
    for i in range(epochs):
        #print('epoch',i)
        precision, recall, fscore, support = predict(model, config, data,i, tag2idx)
